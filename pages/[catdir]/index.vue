...
const limit = ref(9);
const getData = async () => {
  const { data } = await useApi.navContent(catdir, { page: page.value, limit: limit.value });
...
<template>
  <section class="relative px-0 lg:flex-1 lg:px-10 z-[1]">
    <!-- page -->
    ...
    <template v-if="['article', 'album'].includes(catInfo.siteModel)">
      <h2 class="border-b border-gray-100 text-3xl font-medium pb-5 mb-6">{{ catInfo.catname }}</h2>
